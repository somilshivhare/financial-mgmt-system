...
function Dashboard() {
  ...
  const [dashboardTheme, setDashboardTheme] = useState('aurum')
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setDashboardTheme(saved)
    } catch {
      setDashboardTheme('aurum')
    }
  }, [])
  ...
  const monthlySeries = analyticsData?.monthlyInvoicesVsCollections || []
  const outstandingSeries = analyticsData?.outstandingTrends || []

  const widgetDefs = useMemo(() => ([
    { id: 'quickActions', label: 'Quick Actions' },
    { id: 'charts', label: 'Analytics Charts' },
    { id: 'invoiceStatus', label: 'Invoice Status Breakdown' },
    { id: 'recentInvoices', label: 'Recent Invoices Table' },
    { id: 'followUps', label: 'Upcoming Follow-ups' },
    { id: 'overdue', label: 'Overdue Highlights' },
    { id: 'notifications', label: 'Notifications' },
  ]), [])

  const isWidgetVisible = (id) => widgetVisibility[id] !== false
  const toggleWidget = (id) => {
    setWidgetVisibility((prev) => ({ ...prev, [id]: !(prev[id] !== false) }))
  }
  const showAllWidgets = () => {
    const next = {}
    widgetDefs.forEach((w) => { next[w.id] = true })
  ...
  const moduleOverview = useMemo(() => ([
    {
      ...
      label: 'Invoices',
      icon: FileText,
      value: (invoiceInsights.recent || []).length,
      sub: `${invoiceInsights.byStatus?.find((s) => s.status === 'overdue')?.count || 0} overdue`,
      path: '/invoices',
    ...
    },
    {
      ...
      label: 'Payments',
      icon: CreditCard,
      value: Number(kpis.totalCollected || 0),
      sub: 'Total collected',
      path: '/payments',
    ...
    },
    {
      ...
      label: 'PO Entry',
      icon: Package,
      value: Number(kpis.totalBalance || 0),
      sub: 'Current PO value signal',
      path: '/po-entry',
    ...
    },
    {
      ...
      sub: 'Customer masters available',
      path: '/master-data',
    ...
    },
    {
      ...
      label: 'Reports',
      icon: TrendingUp,
      value: Number(kpis.collectionTargetAchieved || 0),
      sub: 'Target achievement %',
      path: '/reports',
      color: chartColors.danger,
      percent: true,
    },
  ]), [invoiceInsights, kpis, customers.length, chartColors])
  ...
  return (
    <div className={`dashboard-page dashboard-theme-${dashboardTheme}`}>
      <div className="dashboard-header">
        <div className="dashboard-header-content">
          <h1 className="dashboard-title">Dashboard</h1>
          <p className="dashboard-subtitle">Customizable command center for all system modules</p>
        </div>
        <div className="dashboard-header-actions">
          <button onClick={() => setShowFilters(!showFilters)} className="dashboard-action-button">
            <Filter className="dashboard-action-icon" />
            <span>Filters</span>
          </button>
          <button onClick={() => setShowWidgetCustomizer(true)} className="dashboard-action-button">
            <Settings2 className="dashboard-action-icon" />
            <span>Customize</span>
          </button>
          <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="dashboard-date-select">
          ...
          <button onClick={handleRefresh} disabled={refreshing} className="dashboard-action-button">
      ...
      {showFilters && (
        <div className="dashboard-filter-card">
          <div className="dashboard-filter-content">
            ...
            <div className="dashboard-filter-select-wrap">
              <select value={customerFilter} onChange={(e) => setCustomerFilter(e.target.value)} className="dashboard-filter-select">
          ...
          <div className="dashboard-theme-row">
            ...
            <div className="dashboard-theme-chips">
              <button type="button" className={`dashboard-theme-chip ${dashboardTheme === 'aurum' ? 'is-active' : ''}`} onClick={() => setDashboardTheme('aurum')}>Aurum</button>
              <button type="button" className={`dashboard-theme-chip ${dashboardTheme === 'ocean' ? 'is-active' : ''}`} onClick={() => setDashboardTheme('ocean')}>Ocean</button>
              <button type="button" className={`dashboard-theme-chip ${dashboardTheme === 'slate' ? 'is-active' : ''}`} onClick={() => setDashboardTheme('slate')}>Slate</button>
            </div>
          </div>
        </div>
      )}
      ...
      <section className="dashboard-hero">
        <div className="dashboard-hero-main">
          <h2>Enterprise Control Tower</h2>
          <p>Full system snapshot across invoices, payments, PO, master data, and collections.</p>
        </div>
        <div className="dashboard-hero-stats">
          <div>
            <span>Outstanding</span>
            <strong>{formatCurrency(kpis.totalOutstanding || 0, currency)}</strong>
          </div>
          <div>
            <span>Collected</span>
            <strong>{formatCurrency(kpis.totalCollected || 0, currency)}</strong>
          </div>
          <div>
            <span>Overdue</span>
            <strong>{formatCurrency(kpis.totalOverdue || 0, currency)}</strong>
          </div>
        </div>
      </section>

      <section className="dashboard-section">
        <h2 className="dashboard-section-title">All Systems</h2>
        <div className="dashboard-system-grid">
          {moduleOverview.map((item) => {
            ...
            return (
              ...
              >
                <div className="dashboard-system-card-top">
                  <div className="dashboard-system-icon" style={{ backgroundColor: `${item.color}20`, color: item.color }}>
                    <Icon size={18} />
                  </div>
                  <span>{item.label}</span>
                </div>
                <div className="dashboard-system-value">{displayValue}</div>
                <div className="dashboard-system-sub">{item.sub}</div>
              </button>
            )
          })}
        </div>
      ...
      <div className="dashboard-widget-grid">
        {isWidgetVisible('quickActions') && quickActions.length > 0 && (
          <section className="dashboard-widget dashboard-widget-span-12">
            <h2 className="dashboard-section-title">Quick Actions</h2>
            <div className="dashboard-quick-actions">
              {quickActions.map((action) => {
                const Icon = action.icon
                return (
                  <button key={action.id} onClick={() => navigate(action.path)} className="dashboard-quick-action-button">
            ...
            <div className="dashboard-charts-grid">
              <div className="dashboard-chart-card">
                <div className="dashboard-chart-head">
                  ...
                  <span className="dashboard-chart-meta">{monthlySeries.length} points</span>
                </div>
                  ...
                  {monthlySeries.length > 0 ? (
                    <ResponsiveContainer width="100%" height={300}>
                      <RechartsLineChart data={monthlySeries}>
                        ...
                        <Legend />
                        <Line type="monotone" dataKey="invoices" name="Invoices" stroke={chartColors.primary} strokeWidth={2.5} dot={{ r: 3 }} />
                        <Line type="monotone" dataKey="collections" name="Collections" stroke={chartColors.success} strokeWidth={2.5} dot={{ r: 3 }} />
                      </RechartsLineChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="dashboard-chart-empty">No analytics data for this period</div>
                  )}
                </div>
              </div>
              <div className="dashboard-chart-card">
                <div className="dashboard-chart-head">
                  ...
                  <span className="dashboard-chart-meta">{outstandingSeries.length} points</span>
                </div>
                <div className="dashboard-chart-canvas">
                  {outstandingSeries.length > 0 ? (
                    ...
                    <div className="dashboard-chart-empty">No outstanding trend data available</div>
                  )}
                </div>
              </div>
            </div>
        ...
        {isWidgetVisible('invoiceStatus') && (
          <section className="dashboard-widget dashboard-widget-span-6">
            <div className="dashboard-card dashboard-insight-card">
              <div className="dashboard-card-header">
                <h3 className="dashboard-card-title">Invoice Status</h3>
                <span className="dashboard-card-kicker">{(invoiceInsights.byStatus || []).reduce((acc, item) => acc + Number(item.count || 0), 0)} total</span>
              </div>
              <div className="dashboard-status-list">
                {(invoiceInsights.byStatus || []).length > 0 ? (invoiceInsights.byStatus || []).map((status) => (
                  <div key={status.status} className="dashboard-status-item">
                    <div className="dashboard-status-indicator" style={{ backgroundColor: statusColors[status.status] || chartColors.gray }} />
                  ...
                  <div className="dashboard-insight-empty">No invoice status data yet</div>
                )}
              </div>
            </div>
          </section>
        ...
        {isWidgetVisible('recentInvoices') && (
          <section className="dashboard-widget dashboard-widget-span-6">
            <div className="dashboard-card dashboard-insight-card">
              <div className="dashboard-card-header">
                <h3 className="dashboard-card-title">Recent Invoices</h3>
                <button onClick={() => navigate('/invoices')} className="dashboard-card-link">View All <ArrowRight className="dashboard-card-link-icon" /></button>
              </div>
                ...
                <table className="dashboard-table">
                  ...
                  <tbody>
                    {filteredRecentInvoices.slice(0, 5).map((invoice) => (
                      <tr key={invoice.id}>
                        <td><button onClick={() => navigate(`/invoices/${invoice.id}`)} className="dashboard-table-link">{invoice.invoice_number}</button></td>
                        <td>{invoice.customer_name || 'N/A'}</td>
                        <td>{formatCurrency(invoice.total_amount || 0, currency)}</td>
                        <td><span className="dashboard-table-status" style={{ backgroundColor: statusColors[invoice.status] || chartColors.gray }}>{invoice.status || 'N/A'}</span></td>
                      </tr>
                    ))}
                    {filteredRecentInvoices.length === 0 && <tr><td colSpan={4} className="dashboard-table-empty">No recent invoices found for selected filters</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
          ...
          <section className="dashboard-widget dashboard-widget-span-6">
            <div className="dashboard-card dashboard-insight-card">
              ...
              <div className="dashboard-list">
                {(paymentsCollections.upcomingFollowUps || []).length > 0 ? (paymentsCollections.upcomingFollowUps || []).slice(0, 5).map((item) => (
                  <div key={item.id} className="dashboard-list-item">
                    ...
                    <div className="dashboard-list-item-meta">
                      ...
                      <div className="dashboard-list-item-date">Due in {item.days_until_due || 0} days</div>
                    </div>
                  </div>
                )) : (
                  <div className="dashboard-insight-empty">No upcoming follow-ups in selected period</div>
                )}
              </div>
            </div>
          </section>
        ...
        {isWidgetVisible('overdue') && (
          <section className="dashboard-widget dashboard-widget-span-6">
            <div className="dashboard-card dashboard-insight-card">
              ...
              <div className="dashboard-list">
                {(paymentsCollections.overdueHighlights || []).length > 0 ? (paymentsCollections.overdueHighlights || []).slice(0, 5).map((item) => (
                  <div key={item.id} className="dashboard-list-item dashboard-list-item-overdue">
                    ...
                    <div className="dashboard-list-item-meta">
                      <div className="dashboard-list-item-value">{formatCurrency(item.balance || 0, currency)}</div>
                      <div className="dashboard-list-item-date dashboard-list-item-date-overdue">{item.days_overdue || 0} days overdue</div>
                    </div>
                  </div>
                )) : (
                  <div className="dashboard-insight-empty">No overdue invoices at the moment</div>
                )}
              </div>
            </div>
          </section>
        ...
        {isWidgetVisible('notifications') && (
          <section className="dashboard-widget dashboard-widget-span-12">
            <div className="dashboard-card">
              <div className="dashboard-card-header">
                <h3 className="dashboard-card-title">Notifications {notificationUnreadCount > 0 && <span className="dashboard-badge">{notificationUnreadCount}</span>}</h3>
                <Bell className="dashboard-card-icon" />
              </div>
              <div className="dashboard-list">
                {(notifications || []).slice(0, 6).map((notification) => (
                  <div key={notification.id} className={`dashboard-list-item ${notification.status === 'new' ? 'dashboard-list-item-unread' : ''}`} onClick={() => markNotificationRead(notification.id)}>
                    <div className="dashboard-list-item-content">
                      <div className="dashboard-list-item-title">{notification.title || notification.message}</div>
                      <div className="dashboard-list-item-subtitle">{notification.type || 'Notification'} • {formatDate(notification.created_at)}</div>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => navigate('/notifications')} className="dashboard-card-footer-link">View All Notifications <ArrowRight className="dashboard-card-link-icon" /></button>
            </div>
          </section>
        )}
      </div>
      ...
      {showWidgetCustomizer && (
        <div className="dashboard-widget-modal-overlay" onClick={() => setShowWidgetCustomizer(false)}>
          <div className="dashboard-widget-modal" onClick={(e) => e.stopPropagation()}>
            <div className="dashboard-card-header">
              <h3 className="dashboard-card-title"><LayoutGrid className="dashboard-card-icon" />Customize Dashboard</h3>
              <button className="dashboard-action-button" onClick={showAllWidgets}>Show All</button>
            </div>
            <div className="dashboard-widget-table-wrap">
              <table className="dashboard-widget-table">
                <thead>
                  <tr>
                    <th>Widget</th>
                    <th>Visible</th>
                    ...
                    <tr key={widget.id}>
                      ...
                      <td>
                        <button
                          ...
                          aria-label={`${isWidgetVisible(widget.id) ? 'Hide' : 'Show'} ${widget.label}`}
                        >
                          {isWidgetVisible(widget.id) ? <CheckSquare className="dashboard-card-icon" /> : <Square className="dashboard-card-icon" />}
                        </button>
                      </td>
                    </tr>
                  ))}
            ...
            <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'flex-end' }}>
              <button className="dashboard-action-button dashboard-action-button-primary" onClick={() => setShowWidgetCustomizer(false)}>Done</button>
            </div>
          </div>
        </div>
      )}
